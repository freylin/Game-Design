...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
		...
		for (int i = 0; i < s.length(); i++) {
			int maxlength = width - EXITOFFSET - length;
			if (maxlength<=0) break;
			int difficulty = random.nextInt(35);
			switch (s.charAt(i)) {
				case 'a':
					length += buildJump(length, maxlength);
					break;
				case 'b':
					length += buildCannons(length, maxlength);
					break;
				case 'c':
					if (maxlength>=10) length += buildHillStraight(length, maxlength, difficulty);
					break;
				case 'd':
					length += buildTubes(length, maxlength, difficulty);
					break;
				case 'e':
					length += buildStraight(length, maxlength, false, difficulty);
					break;
				default:
					break;
			}
	...
	public int buildCannons(int xo, int maxLength)
	{
		...
		for (int x = xo; x < xo + length; x++)
		{
            ...
            int cannonHeight = floor - random.nextInt(2) - 1;
	...
	public int buildHillStraight(int xo, int maxLength, int difficulty)
	{
        ...
        addEnemyLine(xo + 1, xo + 2, floor - 1, difficulty);
	...
	public void addEnemyLine(int x0, int x1, int y, int difficulty)
	{
		for (int x = x0; x < x1; x++)
		{
			if (random.nextInt(35) < difficulty + 1)
			{
				...
				setSpriteTemplate(x, y, new SpriteTemplate(type, false));//Second boolean value determines if enemy is flying
				ENEMIES++;
			}
		}
	}
